...
§3.6-A（半角→全角）と §3.6-B（2文字の全角ラテン）は済んだ。
次は §3.4 の日次パイプライン（GitHub の登録 → `all_years=true`）→ §3.6-C。
...
- **★会議録の英数字は全部全角。** DBに素の半角で問い合わせると0件になる
  ...
  - **`ＡＩ` `ＤＸ` `ＧＸ` `Ｇ７` は2文字語の索引で引く**（2026-08-03・`ROADMAP.md` §3.6-B）。
    2文字なので FTS には入らない。`RUN_PATTERN` に全角ラテンを足して語彙に入れた。
    **この経路だけ大文字に畳む**（すぐ下の「2文字語」を読むこと）
...
- **★この経路だけ全角ラテンを大文字に畳む**（`fold()` / `fold_word_run()` / `toWordKey()`）。
  **FTS 経路の「大文字に寄せない」と矛盾しない。** 別の経路の話で、理由も逆:
  FTS5 は trigram が自分で大小を畳むので寄せると `ＳＤＧｓ` を壊すだけだが、
  `w.term = ?` は BINARY 比較で SQLite が畳まない（NOCASE は ASCII 限定で全角に効かない）。
  畳まないと `ai` が `ａｉ` になって語彙の `ＡＩ` に当たらず「引けない」と出る。
  **畳むのは2文字以下だけ**（3文字以上に掛けると `ＳＤＧｓ`→`ＳＤＧＳ` で本文に当たらない）。
  失うものは無い: 2文字の全角ラテンで小文字を含むのは8語・69発言しかない
- **数字を語彙に入れてある。** `Ｇ７`（4,438発言）`５Ｇ`（615）が引けないと痛い。
  `２０２１` から `２０` `０２` のような組が入る心配は**実測で消えている**
  （会議録は数字を漢数字で書くので、全角数字だけの2文字語は全期間で0件だった）。
...
- **ひらがなだけの2文字語は引けない。** 語彙が漢字・カタカナ・全角ラテンの連続しか見ていないため。

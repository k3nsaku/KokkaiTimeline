...
- **★会議録の英数字は全部全角。半角で引くと0件になる**（`AI` `DX` `LGBT` `TPP` すべて0件、
  `ＡＩ` は5,394件）。**検索窓はまだ未対応**（`ROADMAP.md` §4-10）。
  `ＡＩ` `ＤＸ` `ＧＸ` は2文字なので FTS でも引けず、2文字語の語彙も
  漢字/カタカナしか見ていないため**どちらの索引にも入っていない**。
  → **直し方は決まっている**（§3.6-A / §3.6-B）。**入力を全角に寄せる。
  NFKC は全角→半角の逆方向なので使わないこと**（実測: 議員の発言 512,247件のうち
  半角英字を含むのは54件・0.01%で、寄せて失うものは無い）。
  2文字の全角ラテンは `RUN_PATTERN` を直して語彙に入れる＝**全年DB再構築が要る**。

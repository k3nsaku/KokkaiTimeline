...
「原発」が検索結果0件になる**（`docs/DECISIONS.md`）。日本語の政治キーワードは
...
漢字・カタカナ・全角ラテン/数字のいずれかが**ちょうど2文字だけ連続する**箇所を
「自立した2文字語」と見なし、それが全期間で `--min-df` 件以上の発言に出てくるものを採る。
形態素解析は入れない（依存パッケージを増やさない方針）。

全角ラテンを見るのは `ＡＩ` `ＤＸ` `ＧＸ` `Ｇ７` のため。会議録の英数字は全部全角で、
これらは2文字なので FTS5 の trigram では引けない（`docs/DECISIONS.md`）。
**大小は畳む**（`fold()` の注記を読むこと）。
...
# 漢字の並び・カタカナの並び・全角ラテン/数字の並び。build_topics.py の RUN_PATTERN と
# 揃えてあるが、こちらは長さの上限を置かない（長い連続の中の2文字窓も索引の対象になるため）。
#
# ★ build_db.py の WORD_RUN_PATTERN と**同じもの**にすること。
#   片方だけ変えると、語彙にあるのに索引に入らない語が出る。
#
# 全角ラテンを入れているのは `ＡＩ` 5,394件・`ＤＸ` 3,038件・`ＧＸ` 2,692件・`Ｇ７` 4,438件が
# 2文字で、FTS5 の trigram では**原理的に引けない**ため（`docs/DECISIONS.md`）。
# 数字を含めても増えるのは `Ｇ７` `５Ｇ` `４Ｋ` `３Ｄ` の類だけで、`２０２１` から
# `２０` `０２` のような無意味な組は**1件も入らない**。会議録は数字を漢数字で書くので、
# 全角数字だけの2文字語は全期間で `--min-df` に届くものが0件だった（実測・2026-08-03）。
RUN_PATTERN = re.compile(r"[一-鿿々]{2,}|[ァ-ヴー]{2,}|[Ａ-Ｚａ-ｚ０-９]{2,}")

# 全角ラテンの小文字 → 大文字。**語彙も索引もこれを通した形で持つ。**
#
# FTS 経路と違ってここは大小を畳む。理由: `word` は `w.term = ?` の BINARY 比較で引くので、
# SQLite は畳んでくれない（NOCASE は ASCII 限定で全角に効かない）。畳まないと
# `ai` と打たれた語が `ａｉ` になって語彙の `ＡＩ` に当たらず、「引けない」と出る。
# 失うものは無い: 2文字の全角ラテンで小文字を含むのは8語・69発言（`Ｃｔ` `Ｍｙ` `Ａｉ`
# `ｐＨ` …）しかない。**`ＳＤＧｓ` `ｉＰＳ` `ＩｏＴ` は3文字以上なのでここには来ない**
# （あれらは FTS が引く。`query.ts` の「大文字に寄せない」はそちらの話）。
...
def extract_vocabulary(con: sqlite3.Connection, min_df: int) -> tuple[Counter[str], int]:
    ...
    for (body,) in con.execute("SELECT body FROM speech WHERE speaker_kind = '議員'"):
        ...
        df.update({fold(run) for run in RUN_PATTERN.findall(body) if len(run) == 2})
        if n % 50_000 == 0:

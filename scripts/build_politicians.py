"""Phase 1 §3.2: 名寄せ結果を確定させ、議員マスタと所属の時系列を作る。
...
`docs/ROADMAP.md` §4-2 のとおり、Wikidata に項目が無い議員がいる（2026年の新人が中心）。
...
def resolve_party(kaiha: str | None, party_map: dict, member_parties: list[str],
    ...
    誤った所属は同姓同名の誤分離と同じで、無記入より有害になる（PROJECT_BRIEF §9）。
    割り切れない分は人力で1件ずつ決める。
    ...
    def sync(self, politicians: list[dict], party_map: dict) -> tuple[int, int]:
        ...
        self.path.write_text(json.dumps({
            "_comment": [
                ...
                "（PROJECT_BRIEF.md §9）。1件ずつ根拠を確認して決めること。",
                "",
...
def write_todo(politicians: list[dict], party_map: dict, members_by_id: dict,
    ...
    lines += [
        "",
        "> 会派ごとにまとめて読み替える仕組みは**あえて用意していない**。",
        "> 誤った所属は同姓同名の誤分離と同じで、無記入より有害になる（`PROJECT_BRIEF.md` §9）。",
        "",
        "## 議員ごとの一覧",
        "",

...
### `/politicians` の並び順（`src/lib/table-sort.ts`）

**既定は氏名のよみの五十音順。発言数の多い順ではない。** 発言数は委員会の所属と役職で
決まるので、**運営が既定で多い順に並べると事実上の議員評価になる**
（[docs/SCOPE.md](../docs/SCOPE.md)）。見出しを押せば利用者の側で並べ替えられる。

- **★ビルド時の既定順とブラウザ側の並べ替えは同じ `compareKeys()` から出す。**
  2つ書くと「氏名を2回押すと最初と違う並びになる」が静かに起きる
- **五十音順を自前で作らない。** `Intl.Collator("ja")` を通す。コードポイント順だと
  `が` が `か` の隣に来ず、カタカナ表記の氏名も別の塊になる
- **よみが無ければ氏名で代用する**（空にしない）。ja の照合では漢字がかなの後ろに
  来るので、代用した行はまとまって末尾に寄る
- **空欄は方向によらず末尾**（会派は空がありうる）。降順で先頭に空欄が並ぶと、
  何も分からない行だけが最初に見えることになる
- 押す対象は素の `<button>`、向きは `aria-sort`。列の種類（`text` / `num` / `date`）は
  `<th data-sort>`、行ごとの並べ替えキーは `<td data-v>` に置いてある
  ——**列の定義をフロントマターと `<script>` の2か所に書かないため**
- 同着は**氏名のよみの昇順**で解く。院や会派で並べたとき、同じ会派の中が
  その時々の並びのままだと押すたびに順序が変わって見える

DBを引かないので、絞り込み（`fold()` の照合）と並べ替えはどちらもDOM上で完結する。
テストは `test/table-sort.test.ts`。
...
同じ強さなら発言数の多い順（**検索結果の中での同着の解き方**。一覧の既定順とは別物）。
...
| 条件 | 引き先 | 速さ |
|---|---|---|
| 争点語に一致し、`indexed` が真 | `topic_hit` | 0.7秒。別表記も合算される |
| 2文字以下の語を含む | `word_hit`（本文の2文字窓を全部索引） | FTSでは**原理的に引けない**もの |
| それ以外 | FTS5 trigram | 1.5〜6秒 |

**`indexed` は「配信済みの全期間DBがその語を持っているか」**（`dist/topics.json`）。
争点語を足しても全期間のDBを作り直さなくてよいのはこのため。偽の語は下の2経路に
落ちる（結果は同じで、3文字以上の語だけ 3.3倍遅い）。`matchTopic()` と
`/topic/<id>` の**両方**がこの印を見る。**片方だけ直すと 0件になる**
（持っていない `topic_id` を引くため）。
...
**`hitSource()`** の両方を直す。片方だけだと、一覧の中身と画面上部の件数が
黙って食い違う（実際に会議名の絞り込みが件数に効いておらず、
2,594件と557件が入れ替わっていた）。

`hitSource()` は「ヒットの集合」＝ FROM と WHERE を3経路ぶん1か所で組む関数で、
**件数（`countQuery()`）と月別（`monthlyQuery()`）がこれを共有している**。
以前は件数のSQLを経路ごとに書いていたが、月別を足すときに
「3経路 × 3か所」になるので集約した。

## 検索結果の月別グラフ（`/search`）

検索語が**いつ議論されたか**を棒グラフで出す。描くのは争点語・頻出語のページと
同じ `monthlyChart()`（`src/lib/chart.ts`）で、**既定の縦軸だけが違う**。

- **件数（既定）** — その月に当たった発言数。検索の結果として自然な読み方
- **1,000発言あたり** — 切り替えで出す。その月の総発言数で割る

**同じ語で結論が逆になることがある**ので、件数のときは figcaption で
「国会が長く開かれた月ほど大きく出る」と必ず断る（`docs/DECISIONS.md`）。

引き方の要点:

- **件数はDBから引く**（`monthlyHits()`）。**日付では GROUP BY しない。**
  `speech.rowid` が日付の昇順なので、月の先頭 rowid を境界にして `CASE` で割る。
  索引だけで済み、日付でまとめる書き方より**桁で速い**（`docs/PITFALLS.md`）
- 月境界は `idx_speech_date` の seek で採り、**期間 × 世代でキャッシュする**
  （日次でDBが差し替わると rowid が付き直るので、世代をキーに含める）
- **分母はページに埋め込む**（`topics.json` の `speech_totals` と `meeting_totals`）。
  会議名で絞っているときは**その会議の分母**で割る。全体で割ると率が小さく出る
- 横軸は**検索した年の範囲**に閉じる。範囲外まで並べると、引いてもいない期間が
  「0件だった月」に見える
- 0件のとき・引けない語のときは**セクションごと隠す**
...
- `test/query.test.ts` — 期間DBと同じ形の小さなDBを `node:sqlite` でメモリに作り、
  **`searchQuery()` が返した行数と `countQuery()` が返した数が一致するか**を
  ...
  月別（`monthlyQuery()`）も同じ組み合わせで突き合わせる。**合計が件数と合うだけでなく、
  月の割り当てが日付と一致するか**まで見る（rowid で割っているので、
  ずれても合計は合ってしまう）
- `test/monthly-chart.test.ts` — 月ごとの棒グラフ。**件数と率で棒の高さの大小が
  逆になる**実データ（2025年の `安全保障`）を入れてあり、どちらの数字で
  立てているかを固める
...
- `test/table-sort.test.ts` — 表の並べ替え。**五十音順**（清濁・カタカナ表記・
  よみが無い行）と、空欄が方向によらず末尾に来ることを固める

...
**★発言DBで議員を探させない。** 姓は2文字が多く（`岸田` `林` `高市`）、2文字語は
`word_hit` の語彙に入っていなければ引けない。入っていても返るのは
**その語を含む発言**であって「その議員」ではない。議員マスタは手元にあるので、
照合はページの中で完結させる。
...
2文字語が語彙に無くて `mode: "none"` になる経路も 0件として扱う。
**議員名を打たれたときに通りやすいのはむしろこちら。**

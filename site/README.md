...
**★ 検索語の正規化（半角→全角）はこの3経路の手前、`query.ts` の入口に置く**
（`ROADMAP.md` §3.6-A。**まだ入っていない**）。会議録の英数字は全部全角で、
半角で打つと0件になる。**NFKC を使わないこと** — NFKC は全角→半角に潰す
**逆方向**の正規化で、やると全滅する。必要なのは `A-Za-z0-9` を `+0xFEE0` する写像。
ハイライトに渡す `terms` も変換後の語にする（本文が全角なので、半角のままだと光らない）。
